...
class SHA1:
    ...
    def __str__(self):
        return ''.join("{:>8}".format(hex(h)[2:]).replace(' ', '0') for h in self.__H)
    ...
    def __padding(stream):
        l = len(stream)  # Bytes
        hl = [int("{:>16}".format(hex(l*8)[2:]).replace(' ', '0')[i:i+2], 16)
    ...
    def __debug_print(t, a, b, c, d, e):
        print('t = {0} : \t'.format(t),
              "{:>8}".format(hex(a)[2:]).replace(' ', '0'),
              "{:>8}".format(hex(b)[2:]).replace(' ', '0'),
              "{:>8}".format(hex(c)[2:]).replace(' ', '0'),
              "{:>8}".format(hex(d)[2:]).replace(' ', '0'),
              "{:>8}".format(hex(e)[2:]).replace(' ', '0')
              )
